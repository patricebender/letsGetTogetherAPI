...
io.on("connection", (socket) => {
	const updateAndEmitGame = function (room, emitToSocket) {
	...
	socket.on("guessAnswer", (data) => {
		...
		const diff = round(Math.abs(userAnswer - guess.answer), 2);
	...
	let emitMultiplierCurse = function () {
		io.in(socket.room).clients((error, clients) => {
			...
			const randomPlayer = getRandomPlayers(1, clients)[0];
	...
	const reduceCurseTime = function (game) {
		game.players.forEach((player) => {
	...
	const setNewRandomAdmin = function () {
		io.in(socket.room).clients((error, clients) => {
			if (error) throw error;
			const randomUser = getRandomPlayers(1, clients)[0];
	...
	socket.on("startNewGame", function () {
		...
		const unplayedCardsInGame = cardsLeftInGame(game);
